...
│   ├── ios-device-wda/                  # 自建：用 WebDriverAgent 让 AI 截图、点击、文字触发 Siri 操作 iOS 真机
│   │   ├── SKILL.md
│   │   ├── agents/openai.yaml
│   │   ├── references/                  # cli.md、troubleshooting.md
│   │   └── scripts/wda.py               # python3 标准库；依赖 Xcode 与 libimobiledevice 的 iproxy
...
| `ios-device-wda` | 自建 / 内部整理 | 底层工具 [appium/WebDriverAgent](https://github.com/appium/WebDriverAgent)（不随 skill 分发，首次使用时克隆到 `~/Projects/tools/WebDriverAgent`，钉 `v16.12.10`） | — | 2026-09-24 | 自建；从玩清单 iOS Siri / 小组件真机验收中提炼：iOS 17+ 上 `idevicescreenshot` 报 `Invalid service`、`devicectl` 没有截图和点击，只能靠 WDA。`scripts/wda.py`（python3 标准库）一条 `start` 完成「已在跑就接上 / 否则 build-for-testing（仅首次）→ 后台 test-without-building → iproxy USB 转发 → 建会话」，签名团队自动取当前目录 Xcode 工程的 `DEVELOPMENT_TEAM`；动作命令带 `--then-shot` 在脚本里等待后截图（环境禁止前台 sleep）。实测可用 `siri <文字>` 直接触发 App 快捷指令短语。启动常被 auto 模式当作「运行第三方代码」拦下，内置 `launch-cmd` → 用户终端运行 → `attach` 的兜底；排障表收录 SpringBoard 菜单遇元素查询会收起、按 label 会点到被遮住的同名控件、拼音输入法吞字母等坑 |
