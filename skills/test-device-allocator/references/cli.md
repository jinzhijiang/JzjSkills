...
`allocator_version`、`device_key`、`platform`(android|ios)、`kind`(physical|emulator|simulator)、`device_id`(serial/udid;AVD 未启动时为 null,boot 后回填)、`name`(AVD 名 / 设备名)、`owner_pid`、`project`、`acquired_at`(ISO 带时区)、`ttl_hours`、`created_by_allocator`、`booted_by_allocator`。
...
字段说明:`created`(本次新建的模拟器)、`booted`(本次由脚本启动)、`reused`(幂等复用已持有的锁)、`release_cmd`(可直接执行的释放命令)。
